...
template<typename Real>
__global__
static void _add_vec_mat_point_mul_vec(Real *data, const Real *srcA_data, const Real *srcB_data, MatrixDim dim, MatrixDim dimA, MatrixDim dimB, Real alpha, Real beta,int batch_index)
{
    int32_cuda i = blockIdx.x * blockDim.x + threadIdx.x;
    int32_cuda j = blockIdx.y * blockDim.y + threadIdx.y;
    int32_cuda tgt_index = i + j * dim.stride;
    int32_cuda srcA_index = j + batch_index * dimA.stride;
    int32_cuda srcB_index = i + j * dimB.stride;
    if (i < dimB.cols && j < dimA.cols)
    {
        data[tgt_index] = alpha * srcA_data[srcA_index] * srcB_data[srcB_index] + beta * data[tgt_index];
    }
}
