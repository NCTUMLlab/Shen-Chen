...
template<typename Real>
void CuMatrixBase<Real>::ReshapeFromTensor(const CuTensor<Real> &T, MatrixIndexT mode)
{
...
#endif
  {
    KALDI_LOG << "can not work without GPU";
  }
}

template<typename Real>
void CuTensor<Real>::ReshapeFromTensor(const CuTensor<Real> &T, MatrixIndexT mode)
{
  KALDI_ASSERT(this->num_rows_ >=0 && this->num_cols_ >= 0 &&
...
// sc function
// element of vector multiply rows vector of matrix
// B.dim == dim
template<typename Real>
void CuMatrixBase<Real>::AddVecMatPointMulVec(Real alpha,
    const CuMatrixBase<Real>& A, const CuMatrixBase<Real>& B, Real beta, MatrixIndexT batch_index) 
{
    KALDI_ASSERT(NumRows() == B.NumRows());
    KALDI_ASSERT(NumCols() == B.NumCols());
    KALDI_ASSERT(Stride() == B.Stride());
    KALDI_ASSERT(batch_index < A.NumRows());
#if HAVE_CUDA == 1
    if (CuDevice::Instantiate().Enabled()) 
    {
        Timer tim;
        ...
        GetBlockSizesForSimpleMatrixOperation(NumRows(),NumCols(),
                                              &dimGrid, &dimBlock);
        cuda_add_vec_mat_point_mul_vec(dimGrid, dimBlock, this->data_, A.Data(),
                                       B.Data(), Dim(), A.Dim(), B.Dim(), alpha, beta, batch_index);
        CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());       
    }
    else
#endif
