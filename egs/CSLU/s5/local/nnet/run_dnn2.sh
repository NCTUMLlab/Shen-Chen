...
if [ $stage -le 2 ]; then
  ...
  mmt=0.0
  mmtf=0.5
  dir=exp/dnn4_pretrain-dbn_dnn_lr${lr}_mmt${mmt}
  ali=${gmmdir}_ali
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    steps/nnet/train.sh --feature-transform $feature_transform --dbn $dbn --hid-layers 0 --learn-rate $lr --momentum $mmt \
  ...
  steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" --acwt 0.2 \
    $gmmdir/graph_bg $data_fmllr/test $dir/decode_test || exit 1;
  steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" --acwt 0.2 \
    $gmmdir/graph_bg $data_fmllr/dev $dir/decode_dev || exit 1;
fi
...
#  steps/nnet/make_denlats.sh --nj 4 --cmd "$decode_cmd" --acwt $acwt \
...
#    $data_fmllr/train data/lang $srcdir ${srcdir}_ali ${srcdir}_denlats $dir || exit 1
  # Decode
#  for ITER in 1 6; do
#    steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" \
#      --nnet $dir/${ITER}.nnet --acwt $acwt \
#      $gmmdir/graph_bg $data_fmllr/test $dir/decode_test_it${ITER} || exit 1
#    steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" \
#      --nnet $dir/${ITER}.nnet --acwt $acwt \
#      $gmmdir/graph_bg $data_fmllr/dev $dir/decode_dev_it${ITER} || exit 1
#  done 
#fi
