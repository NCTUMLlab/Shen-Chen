...
if $fake; then
  ...
  ! cat $data/spk2utt | awk -v dim=$dim '{print $1, "["; for (n=0; n < dim; n++) { printf("0 "); } print "1";
    ...
    copy-matrix ark:- ark,t,scp:$cmvndir/cmvn_$name.ark,$cmvndir/cmvn_$name.scp && \
  ...
  ! compute-cmvn-stats-two-channel $data/reco2file_and_channel scp:$data/feats.scp \
       ark,t,scp:$cmvndir/cmvn_$name.ark,$cmvndir/cmvn_$name.scp \
  ...
  ! compute-cmvn-stats scp:$data/feats.scp ark:- | \
    modify-cmvn-stats "$fake_dims" ark:- ark,t,scp:$cmvndir/cmvn_$name.ark,$cmvndir/cmvn_$name.scp && \
    echo "Error computing (partially fake) CMVN stats" && exit 1;
else
  ! compute-cmvn-stats scp:$data/feats.scp ark,t,scp:$cmvndir/cmvn_$name.ark,$cmvndir/cmvn_$name.scp \
