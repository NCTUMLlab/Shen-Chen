...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    ...
    compute-mfcc-feats $vtln_opts --verbose=2 --sample-frequency=8000 --config=$mfcc_config ark:- ark:- \| \
    copy-feats --compress=$compress ark:- \
      ark,t,scp:$mfccdir/raw_mfcc_$name.JOB.ark,$mfccdir/raw_mfcc_$name.JOB.scp \
     || exit 1;
  ...
  $cmd JOB=1:$nj $logdir/make_mfcc_${name}.JOB.log \
    compute-mfcc-feats  $vtln_opts --verbose=2 --sample-frequency=8000 --config=$mfcc_config \
