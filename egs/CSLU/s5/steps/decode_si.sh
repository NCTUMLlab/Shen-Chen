...
if [ $# != 4 ]; then
   echo "Usage: steps/decode.sh [options] <graph-dir> <data-dir> <decode-dir> <it_idx>"
...
case $feat_type in
  ...
  delta) feats="ark,s,cs:apply-cmvn scp:$sdata/JOB/cmvn.scp scp:$sdata/JOB/feats.scp ark:- | add-deltas $delta_opts ark:- ark:- |";;
...
if ! $skip_scoring ; then
  ...
  local/score.sh --cmd "$cmd" $scoring_opts $data $graphdir $dir $4||
