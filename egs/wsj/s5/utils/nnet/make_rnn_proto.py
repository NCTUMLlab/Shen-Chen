...
parser.add_option('--num-hidden', dest='hidden', type='int', default=1024, 
                   help='Number of RNN hidden neurons [default: %default]');
parser.add_option('--rnn-stddev-factor', dest='rnn_stddev_factor', type='float', default=0.01, 
...
if o.num_layers == 1:
    print "<RNNComponent> <InputDim> %d <OutputDim> %d <ParamScale> %f <ClipGradient> %f" % \
        (feat_dim, o.hidden, o.rnn_stddev_factor, o.clip_gradient)
elif o.num_layers == 2:
    print "<RNNComponent> <InputDim> %d <OutputDim> %d <ParamScale> %f <ClipGradient> %f" % \
        (feat_dim, o.hidden, o.rnn_stddev_factor, o.clip_gradient)
    print "<RNNComponent> <InputDim> %d <OutputDim> %d <ParamScale> %f <ClipGradient> %f" % \
        (o.hidden, o.hidden, o.rnn_stddev_factor, o.clip_gradient)
else:
...
print "<AffineTransform> <InputDim> %d <OutputDim> %d <BiasMean> 0.0 <BiasRange> 0.0 <ParamStddev> %f" % \
    (o.hidden, num_leaves, o.param_stddev_factor)
