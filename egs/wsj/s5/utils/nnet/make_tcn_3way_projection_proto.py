...
usage="%prog [options] <in-dim1> <in-dim2> <in-dim3> <out-dim> <hid_dim> >nnet-proto-file"
...
(in_dim1, in_dim2, in_dim3, out_dim, hid_dim) = map(int,args);
### End parse options 
...
print "<TCN3WayProjectionComponent> <InputDim> %d <OutputDim> %d <BiasMean> %f <BiasRange> %f <ParamStddev> %f <InputDim1> %d <InputDim2> %d <InputDim3> %d" % \
  (in_dim1*in_dim2*in_dim3, hid_dim, o.hid_bias_mean, o.hid_bias_range, o.param_stddev_factor, in_dim1, in_dim2, in_dim3)
