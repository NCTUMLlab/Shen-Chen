...
if [ $stage -le 0 ]; then
  # Store fbank pitch features, so we can train on them easily,
  echo "make filter bank pitch features"
  dir=$data
  mkdir -p $data
  for x in train dev test; do
    cp -r data/$x ${data}/$x || exit 1;
    steps/make_fbank_pitch.sh --nj 10 \
      $data/$x exp/make_fbank_pitch/$x $dir || exit 1;
    steps/compute_cmvn_stats.sh $data/$x exp/make_fbank/$x $dir || exit 1;
  done
  # split the data : 90% train 10% cross-validation (held-out)
  utils/subset_data_dir_tr_cv.sh $dir/train $dir/train_tr90 $dir/train_cv10 || exit 1
fi
...
if [ $stage -le 4 ]; then
  ...
  for ITER in 1 6; do
    steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" \
      ...
      $gmmdir/graph $data/test $dir/decode_test_it${ITER} || exit 1
    steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" \
      --nnet $dir/${ITER}.nnet --acwt $acwt \
      $gmmdir/graph $data/dev $dir/decode_dev_it${ITER} || exit 1
  done 
fi
