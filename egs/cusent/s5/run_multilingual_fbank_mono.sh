...
ali_dir_csl="exp/mono_ali,../../timit/s5/exp/mono_ali" # One ali-dir per language,
...
if [ $stage -le 0 ]; then
  ...
  for i in $(seq 0 $[num_langs-1]); do
    ...
    steps/make_fbank.sh --nj 30 --cmd "$train_cmd --max-jobs-run 10" $tgt_dir{,/log,/data}
...
dir=exp/dnn4c-mono-no-cmvn-fbank-multilingual${num_langs}-$(echo $lang_code_csl | tr ',' '-')-${nnet_type} 
[ ! -e $dir ] && mkdir -p $dir
...
if [ $stage -le 2 ]; then  
  case $nnet_type in
    ...
    $cuda_cmd $dir/log/train_nnet.log \
      steps/nnet/train.sh --learn-rate 0.008 \
        --cmvn-opts "--norm-means=false --norm-vars=true" \
...
if [ $stage -le 3 ]; then  
dir=exp/dnn4c-mono-no-cmvn-fbank-multilingual2-cusent-timit-sbn
graphdir=exp/mono
feats=data-fbank/
#feats=/home/shen/kaldi-tensor/egs/CSLU/s5/data-fbank
...
if [ $stage -le 4 ]; then  
dir=exp/dnn4c-mono-no-cmvn-fbank-multilingual2-cusent-timit-sbn
graphdir=../../timit/s5/exp/mono
feats=/home/shen/kaldi-tensor/egs/CSLU/s5/data-fbank
steps/nnet/decode.sh --nj 1 --acwt 0.2 --config conf/decode_dnn.config \
  --blocksoftmax-dims $ali_dim_csl --blocksoftmax-active 2 \
  $graphdir/graph $feats/GE $dir/decode_GE_timit || exit 1;
fi
