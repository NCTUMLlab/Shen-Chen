...
lang_code_csl="cusent,timit" # One label for each language,
lang_weight_csl="1.0,1.0" # Per-language weights, they scale loss-function and gradient, 1.0 for each language is good,
ali_dir_csl="exp/tri3b_ali,../../timit/s5/exp/tri3_ali" # One ali-dir per language,
data_dir_csl="data/train,../../timit/s5/data/train" # One train-data-dir per language (features will be re-computed),
gmm_dir_csl="exp/tri3b,../../timit/s5/exp/tri3"

nnet_type=sbn # dnn_small | dnn | bn
...
gmm_dir=($(echo $gmm_dir_csl | tr ',:' ' '))

# Make sure we have same number of items in lists,
! [ ${#lang_code[@]} -eq ${#ali_dir[@]} -a ${#lang_code[@]} -eq ${#data_dir[@]} -a ${#lang_code[@]} -eq ${#gmm_dir[@]} ] && \
  echo "Non-matching number of 'csl' items: lang_code ${#lang_code[@]}, ali_dir ${ali_dir[@]}, data_dir ${#data_dir[@]}, gmm_dir ${#gmm_dir[@]}" && \
  exit 1
num_langs=${#lang_code[@]}
...
for i in $(seq 0 $[num_langs-1]); do
  ...
  [ ! -d ${gmm_dir[$i]} ] && echo "Missing ${gmm_dir[$i]}" && exit 1
done

# Make the features,
data=data-fmllr-multilingual${num_langs}-$(echo $lang_code_csl | tr ',' '-')
...
if [ $stage -le 0 ]; then
  ...
  for i in $(seq 0 $[num_langs-1]); do
    ...
    fmllr_dir=${tgt_dir}_fmllr
    ali=${ali_dir[i]}
    #utils/copy_data_dir.sh --utt-suffix _$code --spk-suffix _$code $dir $tgt_dir; 
    #rm $tgt_dir/{feats,cmvn}.scp || true # remove features,
    # extract features, get cmvn stats,
    steps/nnet/make_fmllr_feats.sh --nj 8 --cmd "$train_cmd --max-jobs-run 10" \
      --transform-dir $ali \
      $fmllr_dir $dir $gmmdir $fmllr_dir/log $fmllr_dir/data || exit 1
    #steps/compute_cmvn_stats.sh $tgt_dir{,/log,/data}
    # split lists 90% train / 10% held-out,
    utils/subset_data_dir_tr_cv.sh $fmllr_dir ${fmllr_dir}_tr90 ${fmllr_dir}_cv10
    tr90="$tr90 ${fmllr_dir}_tr90"
    cv10="$cv10 ${fmllr_dir}_cv10"
  done
  # Merge the datasets,
...
dir=exp/dnn5a-fmllr-multilingual${num_langs}-$(echo $lang_code_csl | tr ',' '-')-${nnet_type} 
[ ! -e $dir ] && mkdir -p $dir
...
if [ $stage -le 3 ]; then  
dir=exp/dnn5a-multilingual2-cusent-timit-sbn
graphdir=exp/tri3b
feats=data-fbank
steps/nnet/decode.sh --nj 8 --acwt 0.2 --config conf/decode_dnn.config \
  --blocksoftmax-dims $ali_dim_csl --blocksoftmax-active 1 \
  $graphdir/graph $feats/test $dir/decode_cusent || exit 1;
fi
