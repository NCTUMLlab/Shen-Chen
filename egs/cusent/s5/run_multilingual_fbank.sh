...
dir=exp/dnn4b-no-cmvn-fbank-multilingual${num_langs}-$(echo $lang_code_csl | tr ',' '-')-${nnet_type} 
[ ! -e $dir ] && mkdir -p $dir
...
if [ $stage -le 3 ]; then  
dir=exp/dnn4b-no-cmvn-fbank-multilingual2-cusent-timit-sbn
graphdir=exp/tri3b
feats=data-fbank/
...
steps/nnet/decode.sh --nj 1 --acwt 0.2 --config conf/decode_dnn.config \
  --blocksoftmax-dims $ali_dim_csl --blocksoftmax-active 1 \
  $graphdir/graph $feats/MA $dir/decode_MA || exit 1;
fi
